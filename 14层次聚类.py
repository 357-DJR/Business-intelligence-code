...
for experiment in range(3):
    ...
    for train_index, test_index in skf.split(X_pca, y):
        ...
        labels_train = clustering.fit_predict(X_train)
        labels_test = clustering.fit_predict(X_test)
    ...
    avg_davies_bouldin = np.mean(fold_davies_bouldin_scores)
    avg_mutual_info = np.mean(fold_mutual_info_scores)
